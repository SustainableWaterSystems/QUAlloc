...
# modules from the pcrLite model
...
from pcrLite_reporting import  pcrLite_report_initial_conditions
...
class pcrLite_model(object):
    ...
    def initialize(self):
        ...
        self.report_initial_conditions_to_file = pcrLite_report_initial_conditions( \
        ...
        # set default prioritization to all sectors
        ...
        prioritization = None; sector_names = None; source_names = None; withdrawal_names = None
        ...
        del groundwater_allocation_zones,       surfacewater_allocation_zones, \
            ...
            prioritization, sector_names, source_names, withdrawal_names
    ...
    def update(self):
        ...
        for forcing_variable in forcing_variables.keys():
            ...
            # input forcing variables (m/month): precipitation, referencePotET, groundwater_recharge, direct_runoff, interflow
            # input water demands (m/day): irrigation, domestic, industry, livestock, manufacture, thermoelectric, environment
        ...
        for source_name in self.water_management.source_names:
            ...
            if self.model_flags[flag_name]:
                ...
                if date.day == 1 and date.month == 1:
                    ...
                    for var, dtype in [('regional_pumping_limit',pcr.Scalar),('region_ids',pcr.Nominal)]:
                        var_out = read_file_entry( \
        ...
        # get the long-term availability given the date and the model settings
        ...
        for sector_name in self.water_management.sector_names:
            # add demand per sector to the gross and net demand (m3/day)
        ...
        # all forcing data read and processed, assign the water demand to the
        # water management module as total volume per period, per cell
        self.water_management.update_water_demand_for_date( \
                              gross_demand = gross_demand_per_sector, \
                              net_demand   = net_demand_per_sector, \
                              date         = self.model_time.date)
        
        # update environmental water demands if it is evaluated
        # based on the channel storage required to meet the environmental
        # flow requirements (units: m3/day)
        ...
        # set the potential withdrawal per sector (units: m3/day)
        ...
        # [ total runoff ] . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
        # get the channel runoff (units: m/day)
        self.channel_runoff = self.precipitation - \
                             self.surfacewater.water_cropfactor * self.referencepotet
        
        # get the return flow (units: m/day)
        return_flow = self.water_management.total_return_flow / self.cellarea 
        
        # get the total runoff (units: m/day)
        total_runoff = \
        ...
        # get the short-term potential surface water withdrawal per sector (units: m3/day)
        potential_withdrawal_per_sector = \
        ...
        # [ withdrawals ]  . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
        # get the actual renewable and non-renewable groundwater withdrawals (units: m3/day)
        ...
        renewable_withdrawal, nonrenewable_withdrawal, storage = \
                     self.water_management.update_groundwater_actual_withdrawals( \
                               ...
                               total_recharge                           = self.groundwater_recharge, \
        ...
        # groundwater storage in m (per day)
        # surface discharge in m3/s
        # surface water total runoff in m/day
        groundwater_storage    = storage
        surfacewater_discharge = discharge
        surfacewater_runoff     = total_runoff / self.model_time.time_step_length
        
        self.water_management.update_longterm_availability( \
                                    groundwater_storage    = groundwater_storage, \
                                    surfacewater_discharge = surfacewater_discharge, \
                                    surfacewater_runoff     = surfacewater_runoff, \
                                    date                   = self.model_time.date)

# water management module of the pcrLite model
...
path = '/scratch/carde003/pcrLite/_debug'
verbose = False
...
the pcrLite model and manages water use through allocation and withdrawal.
...
as monthly netCDFs:
surfacewater_longterm_availability_ini    : long-term average monthly surface 
                                            water availability [m^3/period]
groundwater_longterm_availability_ini     : long-term average groundwater av-
                                            ailability [m^3/period]
                                            
# and the total return flow needs to be initialized, default is None,
# in which case it is set to zero
total_return_flow_ini:                    : total return flow [m^3/period]


Functions:
==========

To be added! Quite a lot :-|
...
'''
            ...
            if sector_name in self.sector_names:
                self.groundwater_allocation_zones[sector_name]  = \
                        pcr.ifthen(pcr.scalar(groundwater_allocation_zones)>0, \
                                   pcr.nominal(pcr.uniqueid(pcr.ifthen(pcr.scalar(groundwater_allocation_zones)>0, pcr.boolean(1)))))
                # alternative where all groundwater allocation zones are missing values
                # but watch out! all later calculation should be covered with zeros
                #                   pcr.nominal(pcr.ifthen(pcr.scalar(1)==0, pcr.scalar(1)))
        ...
        # sector that can withdraw water only from local sources have higher priority
        for source_name in self.source_names:
        ...
        # compute the total availability
        # set the long-term availability and get the total
            ...
            if self.pumping_capacity_flag[source_name]:
                ...
                if source_name == 'surfacewater':
                    ...
                    self.surfacewater_longterm_pot_withdrawal_dates = \
                                           deepcopy(self.surfacewater_longterm_runoff_dates)
    ...
    def update_total_water_availability(self):
        ...
        # update the long-term total groundwater availability
        ...
        # update the long-term total surface water availability
    ...
    def update_withdrawal_capacity(self, \
        ...
        # convert units of regional pumping limit:
        # from billion cubic meters to cubic meters (units: m3/year)
        regional_pumping_limit = regional_pumping_limit * 1000000000
        
        # define the long-term potential withdrawal per source
        if source_name == 'groundwater':
            longterm_potential_withdrawal = self.groundwater_longterm_potential_withdrawal
        
        if source_name == 'surfacewater':
            longterm_potential_withdrawal = self.surfacewater_longterm_potential_withdrawal
        ...
        # update the variable corresponding to the water withdrawal capacity (units: m3/day)
        if source_name == 'groundwater':
            self.groundwater_withdrawal_capacity = \
                                      regional_pumping_limit * withdrawal_rate / \
                                                               (12 * time_step_length)
    ...
    def update_water_demand_for_date(self, \
        ...
        net_demand    : gross and net demand as a dictionary
                        with the sector names as keys [volume]
                        as a PCRaster map.
    ...
    def get_longterm_availability_for_date(self, \
        ...
        landmask                  : PCRaster map with land mask required to check on missing cells
        ...
        if self.time_increment == 'monthly':
            ...
            # [ groundwater ]
            # get the time step to update the groundwater availability (monthly)
        ...
        # the cell upstream and the total runoff of the same cell (units: m3/s)
        discharge = pcr.upstream(ldd, surfacewater_discharge) + \
                    surfacewater_runoff * cellarea / time_step_seconds
        
        # get the average daily water depth corresponding to this discharge (units: m/day)
        ...
        missing_availability = landmask & (pcr.pcrnot(pcr.defined(groundwater_availability)) |
        ...
        surfacewater_availability = pcr.ifthen(landmask, \
                                               pcr.cover(surfacewater_availability, 0))
        groundwater_availability  = pcr.ifthen(landmask, \
    ...
    def update_environmental_flow_requirements_for_date(self, \
        ...
        # convert environmental flow requirements units from days to seconds (units: m3/s)
    ...
    def update_potential_withdrawals_for_date(self, \
        ...
        landmask     : land mask that is required to check on missing cells
        date         : date of the update
        ...
        withdrawal_per_sector, met_demand_per_sector, sub_message_str = \
               self.allocate_demand_to_renewable_sources( \
                        ...
                        demand_per_sector  = self.gross_demand, \
        ...
        if verbose:
            ...
            for sector_name in self.sector_names:
                pcr.report(self.gross_demand[sector_name],       f'{path}/{dt}_gross_demand_{sector_name}.map')
    ...
    def allocate_unmet_demand_to_nonrenewable_sources(self, \
        ...
        # set variables
        source_name = 'groundwater'
        availability = self.potential_renewable_withdrawal[source_name]
        suitability  = self.suitability_per_sector[source_name]
        zones        = zones_per_sector[source_name]
        
        # filter sectors that can pull water also from groundwater
        ...
        for sector_name in sector_names:
            ...
            # withdrawn
            if not isinstance(withdrawal_capacity[source_name], NoneType):
                capacity = withdrawal_capacity[source_name]
            else:
    ...
    def get_potential_withdrawal(self, \
        ...
        # return the total potential withdrawal (units: m/period)
    ...
    def update_surfacewater_potential_withdrawals(self, \
        ...
        total_runoff                  : PCRaster map with available runoff as the sum of the different
                                       surface water components (units: m/period)
        ...
        # get the short-term potential surface water availability (units: m3/day)
        potential_surfacewater_availability = \
                 (surfacewater_storage * fraction_water + total_runoff ) * cellarea
        
        # get suitability per sector considering short-term surfaace water quality
        ...
        # update long-term potential surface water withdrawals considering 
        # short-term water quality suitability (units: m3/day)
        ...
        # return potential surface water withdrawals per sector for current
        # time-step (units: m3/day)
    ...
    def update_groundwater_actual_withdrawals(self, \
        ...
        total_base_flow                     : PCRaster map with base flow accumulated at the end of the period
                                             (units: m/period)
        longterm_potential_withdrawal_per_sector : 
                                             dictionary with sector names (string) as keys and PCRaster maps
                                             with sum of renewable and non-renewable potential withdrawals per
        ...
        # update the storage with the total recharge and base flow (units: m/period)
        storage = groundwater_storage + total_recharge - total_base_flow
        
        # get the short-term groundwater availability (units: m3/period)
        ...
        # short-term water quality suitability (units: m3/period)
        potential_withdrawal_per_sector = \
            dict((sector_name, \
                  longterm_potential_withdrawal_per_sector[sector_name] * \
                  suitability_per_sector[sector_name] * time_step_length) \
                 for sector_name in self.sector_names)
        
        # aggregate total potential withdrawals from all sectors (units: m3/period)
        ...
        # [ actual withdrawals ]
        # calculate the renewable and non-renewable withdrawals (units: m3/period)
        ...
        # convert renewable and non-renewable withdrawals (units: m3/day)
        ...
        # re-distribute renewable withdrawals by sector (units: m3/day)
    ...
    def allocate_withdrawal_to_demand_for_date(self, date):
        ...
        # initialize the total allocated water, consumption and the return 
        # flow that are updated by iterating over the sectors
        self.total_allocation  = pcr.spatial(pcr.scalar(0))
        self.total_consumption = pcr.spatial(pcr.scalar(0))
        self.total_return_flow = pcr.spatial(pcr.scalar(0))
        ...
        for sector_name in self.sector_names:
            ...
            for key in self.allocated_demand_per_sector.keys():
                ...
                self.consumed_demand_per_sector[key][sector_name] = pcr.max(0, \

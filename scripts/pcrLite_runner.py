...
pcrLite_runner.py: main file that runs the pcrLite mockup model that emulates \
...
from pcrLite_main import pcrLite_model

from pcrLite_reporting import pcrLite_reporting
...
if len(critical_improvements) > 0 or len(development) > 0:
    print ('\nDevelopments for pcrLite_runner:')
...
#==============================================================================
class pcrLite_runner(DynamicModel):
    ...
    def __init__(self, model_configuration, model_time, \
        ...
        self.model = pcrLite_model(self.model_configuration, \
                                   self.model_time, \
                                   model_flags, \
                                   initial_conditions)
        self.reporting = pcrLite_reporting(self.model_configuration)
...
def main():
    ...
    if len(arguments) < 1:
        cfgfilename = 'pcrLite_basic_setup.cfg'
    ...
    pcrLite_instance = pcrLite_runner( \
                                      model_configuration, \
                                      pcr_time, \
                                      model_flags, \
                                      initial_conditions)
    
    pcrLite_model  = DynamicFramework( \
                                      pcrLite_instance, \
                                      lastTimeStep = pcr_time.number_time_steps, \
                                      firstTimestep = 1)
    pcrLite_model.setQuiet(True)
    initial_conditions = pcrLite_model.run()
